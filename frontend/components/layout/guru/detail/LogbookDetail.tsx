...
const LogbookDetailModal: React.FC<LogbookDetailModalProps> = ({
  ...
  useEffect(() => {
    if (entry && isOpen) {
      ...
      if (!finalUrl.startsWith("http://") && !finalUrl.startsWith("https://")) {
        const baseUrl = process.env.NEXT_PUBLIC_LARAVEL_BASE_URL || "http://localhost";
        ...
        // Jika menggunakan local storage path, ubah ke URL yang benar
        if (finalUrl.startsWith("storage/")) {
          // Untuk akses lokal, gunakan base URL + storage path
          finalUrl = `${baseUrl.replace(/\/$/, "")}/${finalUrl}`;
        } else if (finalUrl.includes("public/storage")) {
          // Jika path sudah mengandung public/storage, gunakan langsung
          finalUrl = `${baseUrl.replace(/\/$/, "")}/${finalUrl}`;
        } else {
          // Default ke storage path
          finalUrl = `${baseUrl.replace(/\/$/, "")}/storage/${finalUrl}`;
        }
      }
      ...
      // Beri sedikit delay untuk simulasi loading jika diperlukan
      setTimeout(() => {
        setIsLoading(false);
      }, 300);
    }
  ...
  const handleRetry = () => {
    ...
    // Simulate retry by resetting image source
    if (imageSrc) {
      const newImageSrc = `${imageSrc}?retry=${Date.now()}`;
  ...
  const LoadingDisplay = () => (
    <div className="absolute inset-0 flex flex-col items-center justify-center bg-gray-50">
      <div className="text-center">
        ...
        <p className="text-gray-500 text-sm mt-1">Melalui Next.js Image</p>
      </div>
    </div>
  );
  ...
  const ErrorDisplay = () => (
    <div className="absolute inset-0 flex flex-col items-center justify-center bg-red-50 border-2 border-dashed border-red-200 rounded-lg p-4">
      ...
      <p className="text-red-500 text-sm mt-1 text-center max-w-xs">
        {imageSrc?.includes('localhost') || imageSrc?.includes('127.0.0.1') 
          ? "Pastikan server Laravel berjalan di localhost" 
          ...
          <>
            ...
            <button
              onClick={async () => {
                ...
                try {
                  console.log("🔗 Testing URL:", imageSrc);
                ...
                {isLoading ? (
                  ...
                  <div className="relative w-full h-full">
                    <Image
                      ...
                      onError={() => {
                        console.error("❌ Next Image load error for:", imageSrc);
                        setImageError(true);
                      }}
                      onLoad={() => {
                      ...
                      loading="lazy"
                      unoptimized={true} // Nonaktifkan optimisasi karena gambar dari sumber eksternal
                      ...
                      <div className="mt-2 p-3 bg-white rounded border text-xs space-y-2">
                        ...
                        <div className="grid grid-cols-2 gap-2">
                          ...
                          <div>
                            ...
                            <p className="text-gray-600">✅ Next.js Image</p>
                          </div>
                        </div>
                        ...
                        <div>
                          ...
                          <p className="text-gray-600">
                            {imageSrc?.startsWith('http://localhost') 
                              ? "✅ Localhost - Pastikan ditambahkan di next.config.js" 
                              : imageSrc?.startsWith('http://127.0.0.1')
                              ? "✅ Local IP - Pastikan ditambahkan di next.config.js"
                              : "⚠️ Periksa next.config.js untuk domain ini"}
                          </p>
                        </div>
                        ...
                        {imageError && (
                          <div className="pt-2 border-t bg-red-50 p-2 rounded">
                            ...
                            <ul className="text-red-600 space-y-1 text-xs list-disc list-inside">
                              <li>Check if Laravel backend is running on localhost:8000</li>
                              <li>Verify image URL is correct and accessible</li>
                              <li>Add domain to next.config.js images.domains array</li>
                              <li>Ensure storage is linked (php artisan storage:link)</li>
