...
type StatusMagang = 'pending' | 'diterima' | 'ditolak' | 'berlangsung' | 'selesai' | 'dibatalkan';
    ...
    const fetchStatusMagang = async () => {
        try {
            ...
            if (result.success && result.data) {
                const validStatus: StatusMagang[] = ['pending', 'diterima', 'ditolak', 'berlangsung', 'selesai', 'dibatalkan'];
    ...
    if (loading) {
        return (
            <div className="flex items-center justify-center min-h-[400px]">
                ...
                <span className="ml-3 text-gray-600">Memuat status magang...</span>
            </div>
        );
    }
        ...
        <div className="p-6 space-y-6">
            <div className="flex flex-col sm:flex-row sm:items-center sm:justify-between gap-4">
                <div>
                    ...
                    <p className="text-gray-600 mt-1">Catat kegiatan dan kendala magang Anda setiap hari</p>
                </div>
                
                {/* Info Status Magang */}
                <div className="text-left sm:text-right">
                    <p className="text-sm text-gray-600">Status Magang:</p>
                    <span className={`inline-block px-3 py-1 rounded-full text-sm font-semibold mt-1 ${getStatusClass(statusMagang)}`}>
